  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-session', (data) => {
      ...
      if (hasJoined.success) {
        ...
        const players = multiplayerQuizManager.getPlayersFromSession(data.sessionId)

        console.log('[SERVER] quiz-joined-success emitted')

        socket.emit('quiz-joined-success', { quizId: hasJoined.quizId, sessionId: data.sessionId, players: players })

        console.log('[SERVER] player-has-joined-session emitted')

        const adminSocketId = multiplayerQuizManager.getAdminSocketId(data.sessionId)
        io.to(adminSocketId).emit('player-has-joined-session', { sessionId: data.sessionId, profile: data.profile })
        multiCast('player-has-joined-session', data.sessionId, { sessionId: data.sessionId, profile: data.profile })
      } else {
    ...
    socket.on('submit-answer', (data) => {
      const isValid = multiplayerQuizManager.checkDataValidity(data.sessionId, data.profile.id, data.answer)
      if (isValid) {
    ...
    socket.on('disconnect', () => {
      ...
      setTimeout(() => {
        if (!reconnected) {
          ...
          const searchAdminResult = multiplayerQuizManager.isSessionAdmin();
          if (searchAdminResult.isAdmin) {
            console.log('[SERVER] An admin has left the session');
            console.log('[SERVER] Kicked emitted');
            multiCast('kicked', searchAdminResult.sessionId, { sessionId: searchAdminResult.sessionId });
          } else {
