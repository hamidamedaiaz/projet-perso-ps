...
export class ProfileStatsService {
  ...
  constructor() { this.initializeCache(); }



  getProfileStats(profileId: number): ProfileStatsBasic {
    if (this.profileStatsCache.has(profileId)) return this.profileStatsCache.get(profileId)!;
  ...
  clearCache(profileId?: number): void {
    if (profileId !== undefined) this.profileStatsCache.delete(profileId); 
    else {
